...
class OrderService:
    ...
    def process_order(items: List[CartItem], payment_method: str, customer_id: Optional[int] = None) -> int:
        ...
        with get_db_connection() as conn:
            ...
            # ۴. در صورت نسیه بودن (credit)، ثبت در جدول debt_transactions و به‌روزرسانی current_debt مشتری
            if customer_id and payment_method == 'credit':
                # الف) ثبت لاگ در debt_transactions
                cursor.execute("""
                ...
                # ب) افزایش مستقیم بدهی در ستون current_debt جدول customers (علت اصلی عدم نمایش در UI)
                cursor.execute("""
                    UPDATE customers
    ...
    def get_active_orders() -> List[Dict[str, Any]]:
        ...
        with get_db_connection() as conn:
            ...
            for row in rows:
                ...
                # دریافت اقلام مرتبط با هر سفارش
                cursor.execute("""
                ...
                # ساخت فیلد items_summary که UI بهش نیاز داره (رفع باگ KeyError)
                if items_list:
